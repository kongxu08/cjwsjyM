...
import static com.cjwsjy.app.SmApplication.sp;
 ...
 */
abstract public class BaseListActivity3 extends BaseActivity2 implements ItemViewHandler{
	...
	//	map.put("rows", pageSize);
		...
		if(requestId==1001)
		{
			...
			username = sp.getString(Constants.sp_loginName, "");
			String strurl = url+"?status=1&CurrentLoginUserAccount="+username+"&current_page=1";
			//String strurl = url+"?status=1&CurrentLoginUserAccount=xuweili&current_page=1";
			doRequest3(1098, strurl, map, 1 );
		}
		else
		{
    ...
    class ReqThread3 extends Thread
    {
        ...
        public void run()
        {
			...
			{
				resultStr = HttpClientUtil.HttpUrlConnectionPost5(strurl, dataMap,"UTF-8",102);
			}
            length = resultStr.length();
            if( length==0 )
            {
