...
public class ActivityYewu extends BaseActivity implements OnItemClickListener
{
	...
	public ListView initList() 
	{
		...
		listItem1.setIv_icon1(R.drawable.home_yewu02);
		listItem1.setTv_title("(政务)接待申请");
		listItems.add(listItem1);

		FinishPeddingItem listItem4 = new FinishPeddingItem();
		listItem4.setIv_icon1(R.drawable.home_yewu04);
		listItem4.setTv_title("(商务，外事)接待申请");
		listItems.add(listItem4);

		FinishPeddingItem listItem2 = new FinishPeddingItem();
		listItem2.setIv_icon1(R.drawable.home_yewu01);
		listItem2.setTv_title("业务接待待办");
		listItem2.setTv_date("0");
		listItems.add(listItem2);

		FinishPeddingItem listItem3 = new FinishPeddingItem();
		listItem3.setIv_icon1(R.drawable.home_yewu03);
		listItem3.setTv_title("业务接待已办");
		listItems.add(listItem3);
		
		// 生成适配器的Item和动态数组对应的元素
		listItemAdapter = new AdaperBaoxiao( ActivityYewu.this, listItems );
		// 添加并且显示
		...
		list.setOnItemClickListener(new OnItemClickListener() 
		{
			...
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) 
			{
				...
				if( position==0 )
				{
					m_threadlog.setparm2("点击", "(政务)接待申请", "(政务)接待申请-业务接待-首页");
				...
				{
					m_threadlog.setparm2("点击", "(商务、外事)接待申请", "(商务、外事)接待申请-业务接待-首页");
					m_threadlog.writelog();
				...
				{
					m_threadlog.setparm2("点击","业务接待待办", "业务接待待办-业务接待-首页");
					m_threadlog.writelog();
				...
				{
					m_threadlog.setparm2("点击","业务接待已办", "业务接待已办-业务接待-首页");
					m_threadlog.writelog();

					url = appUrl+"/CEGWAPServer/JD/getUserYBList/"+m_loginname+"/"+nyear;
					startWebView(url,"业务接待已办");
				}
			}
		});
	...
	private void Getdaiban()
	{
		...
		try
		{
			String url = appUrl + "/CEGWAPServer/JD/getUserDBCount/" + m_loginname;
			...
			for (i = 0; i < length; i++)
			{
				...
				bresult = titlestr.equals("业务接待待办");
				if( bresult==true )
				{
					m_huiyi1 = countstr;
					continue;
	...
	private void startWebView(String urlstr,String title)
	{
		...
		intent.setClass( ActivityYewu.this, WebViewHome.class );
