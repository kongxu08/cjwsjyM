...
public class AdaperDangan extends BaseAdapter
{
	...
	private ActivityLixian m_activityDangan;

	public AdaperDangan(Context context, List<FinishPeddingItem> list)
	{
		inflater = LayoutInflater.from(context);
		this.listItems = list;
		this.mContext = context;
		m_activityDangan = (ActivityLixian)context;
	...
	public View getView(final int position, View convertView, ViewGroup parent)
	{
		...
		if(convertView==null)
		{
			convertView=inflater.inflate(R.layout.listitem_dangan, parent, false);
			...
			holder.iv_button1=(ImageView) convertView.findViewById(R.id.button1);
			holder.iv_button2=(ImageView) convertView.findViewById(R.id.button2);
		...
		//重命名按钮
		holder.iv_button1.setOnClickListener(new View.OnClickListener()
		...
		//删除
		holder.iv_button2.setOnClickListener(new View.OnClickListener()
				//holder.btn_replace.setOnClickListener(new View.OnClickListener()
		{
			@Override
			public void onClick(View v) {
	...
	public void Delete(int position)
	{
		...
		//m_activityDangan.updateList(position);
	}
	...
	//重命名
	private void showNoticeDialog(String strname)
	{
		Button btn_ok;
		Button btn_cancel;
		...
		btn_ok.setOnClickListener(new View.OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				FinishPeddingItem data = (FinishPeddingItem) getItem(m_nposition);

				//数据库中的id
				strId = data.getTv_formid();
				//新名字
				strname = et_text.getText().toString();
				nresult = strname.length();
				if( nresult==0 )
				{
				...
				//查看是否重名
				nresult = db.findDanganByNewName(strname);
				//nresult = 1;
				if(nresult==1)
				{
					noticeDialog.dismiss();
	...
	private void showNoticeDialog2(String strname)
	{
		...
		LayoutInflater inflater = LayoutInflater.from(mContext);
		View viewDialog = inflater.inflate(R.layout.dialog_custom5, null);

		et_text2 = (TextView)viewDialog.findViewById(R.id.EditText_msg);
		//strtext = "确定要删除此文件吗？\r\n"+strname;
		strtext = "确定要删除此文件吗？";
		et_text2.setText(strtext);
		...
		btn_ok.setOnClickListener(new View.OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				FinishPeddingItem data = (FinishPeddingItem) getItem(m_nposition);
				strId = data.getTv_formid();
				strname = et_text2.getText().toString();

				//strname = db.findDanganByNewName(m_strfilename);
				strname = db.findDanganById(strId);

				String sdPath = Environment.getExternalStorageDirectory() + "/Download" + "/com.cjwsjy.app/DangAn/"+strname;
				...
				//修改数据库
				db.DeleteDangan(strId);

				//刷新界面
				m_activityDangan.updateList(m_nposition);

				//修改listview列表的值
				//deleteItem(m_nposition,strname);
	...
	private void deleteItem(int index,String strname )
	{
		if (mListView == null)
		{
			return;
	...
	private void updateItem(int index,String strname )
	{
		...
		// 更新的数据
		FinishPeddingItem data = (FinishPeddingItem) getItem(index);
		//String name = data.getTv_title();  //测试时用
		data.setTv_title(strname);
	}
